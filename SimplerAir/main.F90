...
! filename
!
  character(*), parameter     :: file_in = 'Afivo_data.dat', file_out = 'ZDPlaskin_simplerAir.dat'
...
!
  ...
  double precision :: dtime, time, e_density, electric_fld, photo, N2_plus,&
                      O, O2_min, &
                      O2_plus, O_min
  integer, parameter :: n_columns = 9
...
!
  ...
  idata_max = 0
  do while( .true. )
    read(1,*,end=11) time, e_density, electric_fld, photo, N2_plus, O, O2_min, &
                     O2_plus, O_min
...
!
  ...
  call ZDPlasKin_set_density(   'N2^+', data_in(1, 5))
  call ZDPlasKin_set_density(   'O2^+', data_in(1,8))
...
!
  ...
  write(1,'(99A14)') 'TIME', ( trim(species_name(i)), i = 1, species_max )
  write(*,'(99A14)') 'TIME'
